...
namespace SpatialElementGeometryCalculator
{
  class Util
  {
    ...
    /// </summary>
    public double AreaOfTriangle(XYZ pt1, XYZ pt2, XYZ pt3)
    {
    ...
    /// </summary>
    public static double sqFootToSquareM(double sqFoot)
    {
    ...
    public static Solid UniteSolids(List<Solid> solidsToMerge)
    {

        if (solidsToMerge.Count == 1)
        {
            return solidsToMerge[0];
        }

        Solid unitedSolid = BooleanOperation_Union(solidsToMerge[0], solidsToMerge[1]);

        if (solidsToMerge.Count > 2)
        {
            for (int i = 2; i < solidsToMerge.Count; i++)
            {
                BooleanOperation_Union(ref unitedSolid, solidsToMerge[i]);
            }
        }
